...
if($("#lineupStateMessage"))$("#lineupStateMessage").textContent="Connecting to Firebase active-season data...";
...
async function load(){
  ...
  if(!season.exists())throw new Error(`Active season ${seasonId} was not found in Firebase.`);
...
function renderBuilderContext(){
  ...
  if(!data.teams.length||!data.matchups.length||!data.assignments.length)setMessage(`Firebase data incomplete: ${data.teams.length} teams · ${data.matchups.length} matchups · ${data.assignments.length} roster assignments.`);
...
async function loadApproverAdmin(){
  ...
  select.innerHTML='<option value="">Loading seasons...</option>';if(message)message.textContent="Loading seasons and registered players from Firebase...";
  try{
  ...
  }catch(error){select.innerHTML='<option value="">Unable to load seasons</option>';if(message)message.textContent=`Firebase season load failed: ${error.message}`;throw error;}
}
...
window.addEventListener("alphaopen:profile-ready",()=>{if(window.alphaOpenLocalDevelopment){const select=$("#lineupApproverSeason"),message=$("#lineupApproverMessage");if(select)select.innerHTML='<option value="">Firebase disabled in local development</option>';if(message)message.textContent="Open the deployed AlphaOpen site to manage live Firebase season approvers.";return;}load().catch(showLoadError);});
