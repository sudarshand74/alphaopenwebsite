...
assert(html.includes('runtime-loader.js?v=106')&&runtimeLoader.includes('firebase-data.js?v=92')&&serviceWorker.includes('/firebase-data.js?v=92'),"Current Season refresh modules must use consistent cache-busted versions");
...
assert(html.includes('app.js?v=155')&&serviceWorker.includes('/app.js?v=155'),"Simplified EC menu highlighting must use consistent cache-busted app versions");
assert(html.includes('weekly-lineup-dashboard.css?v=2')&&serviceWorker.includes('/weekly-lineup-dashboard.css?v=2'),"Weekly dashboard styles must use consistent cache-busted versions");
...
assert(js.includes("View Fully Approved Weekly Lineups by Team and current status of those matches"),"Weekly dashboard Home action must use the requested description");
...
assert(/weeklyDashboardStatusButton\("all", "All", rows\.length\)[\s\S]*weeklyDashboardStatusButton\("completed", "Completed", counts\.completed\)[\s\S]*weeklyDashboardStatusButton\("scheduled", "Scheduled", counts\.scheduled\)[\s\S]*weeklyDashboardStatusButton\("toBeScheduled", "To Be Scheduled", counts\.toBeScheduled\)/.test(js),"Weekly status filters must appear as All, Completed, Scheduled, and To Be Scheduled");
assert(!js.includes('weeklyDashboardStatusButton("upcoming", "Upcoming"'),"Weekly status filters must not include the redundant Upcoming option");
assert(js.includes('label: `${fullTeamName(home.teamId, home.name, teamMap)} - ${weeklyDashboardCaptain(home)}`')&&js.includes('label: `${fullTeamName(away.teamId, away.name, teamMap)} - ${weeklyDashboardCaptain(away)}`'),"Weekly captain options must use Team Name - Captain Name without public Home/Away labels");
...
assert(js.includes("alphaOpenLoadWeeklyLineupIndex")&&js.includes("alphaopen:weekly-index:")&&js.includes("firestore.googleapis.com/v1/projects/"),"Weekly Dashboard must preload and cache only its public active-season index");
assert(firebaseData.includes("window.alphaOpenLoadWeeklyLineupIndex")&&firebaseData.includes("weeklyLineupIndex = active"),"Firebase weekly loading must reuse the lightweight public-index preload");
...
assert(/setDoc\(PUBLIC_ACTIVE_SEASON_REF,[\s\S]*?\}, \{merge: true\}\)/.test(firebaseData),"Refreshing the active season must preserve public Week and Captain dashboard metadata");
...
assert(runtimeLoader.includes('users:"./operations-access-admin.js?v=3"'),"User Management must lazy-load the Operations access Admin screen");
...
assert(operationsAccessAdmin.includes("playerMasterEmail || grantEmail"),"Operations access editor must display the current Player Master email before a stale grant snapshot");
assert(operationsAccessAdmin.includes("originalAccessId || accessDocumentId(originalEmail)"),"Operations access email migration must revoke the exact original grant document");
assert(operationsAccessAdmin.includes("Previous approval:")&&operationsAccessAdmin.includes("Provision new email"),"Revoked email grants must show the current Player Master email and a clear reprovision action");
...
assert(venueAdmin.includes('updateDoc(doc(db, "venues", editingVenueId)'),"Venue edits must save to the existing Venue Master record");
assert(venueAdmin.includes("function openAddVenue()")&&venueAdmin.includes("transaction.set(venueRef")&&venueAdmin.includes("already exists in Venue Master"),"Venue Master must create new venues without overwriting duplicate Venue IDs");
assert(html.includes('id="addVenue" type="button">+ Add Venue</button>'),"Venue Master must show an Add Venue action");
...
assert(runtimeLoader.includes('venues:"./venue-admin.js?v=29"')&&serviceWorker.includes('/venue-admin.js?v=29'),"Venue Master create access must use consistent cache-busted versions");
