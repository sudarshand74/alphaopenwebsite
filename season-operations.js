...
function renderCaptainReadiness(){const panel=$("#captainReadiness");if(!panel||!state)return;panel.innerHTML=state.teams.map(team=>`<div class="season-admin-row"><div><b>${esc(team.name)}</b><small>${esc((team.captainPlayerIds||[]).join(", "))}</small></div><span class="badge ${(team.captainPlayerIds||[]).length?"lime":"orange"}">${(team.captainPlayerIds||[]).length?"Captain assigned":"Captain not assigned"}</span></div>`).join("");}
